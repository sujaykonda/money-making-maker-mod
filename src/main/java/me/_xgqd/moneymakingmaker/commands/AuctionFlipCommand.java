                            ...
                            public void accept(String s, ItemAuctionData auctions) {
                                if (data.binData.containsKey(s) && data.binData.get(s).size() >= 20) {
                                    ...
                                    while (i < auctions.size()
                                            && auctions.getPrice(i) < data.binData.get(s).getPrice(0)) {
                                        if (auctions.get(i).getTimeLeft() < 180000
                                                && auctions.get(i).getTimeLeft() > 1000) {
                        ...
                        for (int i = 0; i < 5; i++) {
                            if (bestFlips.size() == i) {
                                ChatComponentText text = new ChatComponentText(
                                        "Sorry, could not find any more auctions");
                                s.addChatMessage(text);
                                break;
                            }
                            String auctioneer_uuid = data.auctionData.get(bestFlips.get(i).itemId)
                                    .get(bestFlips.get(i).i).getAuctioneerUUID();
                            System.out.println(auctioneer_uuid);
                            String auctioneer_name = data.auctionData.get(bestFlips.get(i).itemId)
                                    .get(bestFlips.get(i).i).getAuctioneerName();
                            System.out.println(data.auctionData.get(bestFlips.get(i).itemId).get(bestFlips.get(i).i)
                                    .getTimeLeft());
