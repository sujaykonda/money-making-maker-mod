...
public class Hypixel {
    ...
    public AuctionHouseData getAuctionHouseData() throws IOException {
        ...
        int totalPages = Request
                .readJsonFromUrl("https://api.hypixel.net/skyblock/auctions?key=" + key)
                .getAsJsonObject().get("totalPages").getAsInt();
        for (int i = 0; i < totalPages; i++) {
            JsonArray auctions = Request.readJsonFromUrl(
                    "https://api.hypixel.net/skyblock/auctions?key=" + key + "&&page="
                            + i)
                    .getAsJsonObject().getAsJsonArray("auctions");
            for (JsonElement o : auctions) {
                if (!o.getAsJsonObject().get("claimed").getAsBoolean()
                        && o.getAsJsonObject().get("end").getAsLong() > System.currentTimeMillis() + 6000) {
